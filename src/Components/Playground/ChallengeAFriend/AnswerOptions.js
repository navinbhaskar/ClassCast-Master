...
import {markOption, setAnswers} from "../../../Redux/Playground/Challenge";
...
class AnswerOptions extends Component<{}> {
  selectOption = option => {
    ...
    if (this.props.challenge.reviewMode) {
      return;
    }
    this.props.markOption({
      questionIndex: this.props.challenge.currentIndex,
      optionIndex: option
    })
  ...
  checkAnswer = () => {
    const {currentIndex, answers, correctAnswers} = this.props.challenge;
    return (
  ...
  componentDidMount() {
    if (!this.props.challenge.answers.length) {
      this.props.setAnswers({
        answers: this.props.challenge.qa.map(() => -1),
        correctAnswers: this.props.challenge.correctAnswers,
      });
    }
  }

  render() {
    const {currentIndex, qa, reviewMode, answers, correctAnswers} = this.props.challenge;
    return (
        ...
        }
        {
          qa[currentIndex].answers.map((option, index) => (
            <Option
              ...
              review={this.props.challenge.reviewMode}
              showAsCorrect={this.props.challenge.reviewMode && correctAnswers[currentIndex] === index}
            />
          ))
        }
      </ScrollView>
